...
class PuppeterHcaptchaSolve {
    ...
    async solve(page: Page) {
        let isElmPresent = await this._detect_captcha(page)
        if (isElmPresent) {
            ...
            if (frame !== null) {
                ...
                let _challenge_question = await frame.evaluate(el => el.textContent, elm);
                let _label = await this._get_label(_challenge_question);
        ...
        if (cursor != null) {
            setTimeout(async () => {
                for (let i = 0; i < 9; i++) {
                    ...
                    let style: string = await frame.evaluate((el) => el.getAttribute("style"), elementHandle)
                    let url = style.split('url("')[1].split('")')[0];
                    let res = await get_result(url, label)
                    if (res == "true") {
